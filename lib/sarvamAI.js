 ...
 * 
 * Provides a unified interface for Sarvam AI chat completions.
...
const SARVAM_BASE_URL = process.env.SARVAM_AI_BASE_URL || 'https://api.sarvam.ai';
...
export async function sarvamChatCompletion({
  ...
  if (wiki_grounding) {
    requestBody.wiki_grounding = wiki_grounding;
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Sarvam AI API error (${response.status}): ${errorData.error?.message || response.statusText}`
      );
    }
 ...
 * @param {number} options.num_predict - Maximum tokens (maps to max_tokens)
